...
> 单账号单会话功能上线后，旧版本 JWT 因为不含 `sid` 会自动失效，所有已登录账号需要重新登录一次。
...
| 员工 | GET | `/api/employees/me/earnings` | 查询已完成服务的累计收入 |
...
| 员工 | PATCH | `/api/employees/me/appointments/{id}/cancel` | 取消预约，必须提交原因 |
...
curl -X POST http://localhost:8080/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{"role":"EMPLOYEE","username":"nurse1","password":"123456","name":"王护工","phone":"13900000000","age":30}'
```
...
curl -X POST http://localhost:8080/api/appointments \
  ...
  -H "Content-Type: application/json" \
  -d '{"employeeId":1,"serviceId":1,"appointmentDate":"2026-07-15","timePeriods":["MORNING","AFTERNOON"]}'
```
...
**8. 护工填写原因并取消预约**(用员工 token)
```bash
curl -X PATCH http://localhost:8080/api/employees/me/appointments/1/cancel \
  -H "Authorization: Bearer <员工token>" \
  -H "Content-Type: application/json" \
  -d '{"reason":"临时身体不适"}'
```
